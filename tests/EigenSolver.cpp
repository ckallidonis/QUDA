...
int main(int argc, char **argv)
{
  ...
  double __complex__ *evecs = (double __complex__ *) malloc(ncv*xdim*ydim*zdim*tdim*12*sizeof(double __complex__ )); //note: no extra buffer

  double __complex__ *evals = (double __complex__ *) malloc(ncv*sizeof(double __complex__ ));

  if( evecs == NULL) errorQuda("Error: not enough memory for eigenVectors");
  if( evals == NULL) errorQuda("Error: not enough memory for eigenValues");


  eig_param.invert_param=&inv_param;

  //  eig_param.MatPoly_param[0]= 1.0e-2;
