...
//-C.K. x_id[4] are the local coords each thread corresponds to
r1 = sid / c_localL[0];
...
//-C.K. Indices for double hops: plus-plus, plus-minus, minus-plus and minus-minus points
int FptPP, FptPM, FptMP, FptMM; // For the forward propagator
int SptPP, SptPM, SptMP, SptMM; // For the sequential propagator
int GptPP, GptPM, GptMP, GptMM; // For the gauge field

//-C.K. Indices for single hops
int FptP, FptM; // For the forward propagator
int SptP, SptM; // For the sequential propagator
int GptP, GptM; // For the gauge field
