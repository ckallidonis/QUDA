...
void QKXTM_Contraction_Kepler<Float>::writeTwopBaryonsHDF5_MomSpace(void *twopBaryons, char *filename, qudaQKXTMinfo_Kepler info, int isource){
  ...
  if(info.CorrSpace!=MOMENTUM_SPACE && info.HighMomForm) errorQuda("writeTwopBaryonsHDF5_MomSpace: Support for writing the Baryon two-point function only in momentum-space for low # of momenta!\n");
...
void QKXTM_Contraction_Kepler<Float>::writeTwopBaryonsHDF5_MomSpace_HighMomForm(void *twopBaryons, char *filename, qudaQKXTMinfo_Kepler info, int isource){
  ...
  if(info.CorrSpace!=MOMENTUM_SPACE && !info.HighMomForm) errorQuda("writeTwopBaryonsHDF5_MomSpace_HighMomForm: Support for writing the Baryon two-point function only in momentum-space for high # of momenta!\n");
...
template<typename Float>
void QKXTM_Contraction_Kepler<Float>::copyTwopBaryonsToHDF5_Buf(void *Twop_baryons_HDF5, void *corrBaryons, int isource, CORR_SPACE CorrSpace, bool HighMomForm){
...
void QKXTM_Contraction_Kepler<Float>::writeTwopMesonsHDF5_MomSpace(void *twopMesons, char *filename, qudaQKXTMinfo_Kepler info, int isource){
  ...
  if(info.CorrSpace!=MOMENTUM_SPACE && info.HighMomForm) errorQuda("writeTwopMesonsHDF5_MomSpace: Support for writing the Meson two-point function only in momentum-space and for low # of momenta!\n");
...
void QKXTM_Contraction_Kepler<Float>::writeTwopMesonsHDF5_MomSpace_HighMomForm(void *twopMesons, char *filename, qudaQKXTMinfo_Kepler info, int isource){
  ...
  if(info.CorrSpace!=MOMENTUM_SPACE && !info.HighMomForm) errorQuda("writeTwopMesonsHDF5_MomSpace_HighMomForm: Support for writing the Meson two-point function only in momentum-space and for high # of momenta!\n");
  ...
  if(GK_timeRank >= 0 && GK_timeRank < GK_nProc[3] ){
    ...
    //-Write the momenta in a separate dataset
    if(GK_timeRank==sink_rank){
      hid_t file_idt   = H5Fopen(filename, H5F_ACC_RDWR, H5P_DEFAULT);
...
template<typename Float>
void QKXTM_Contraction_Kepler<Float>::copyTwopMesonsToHDF5_Buf(void *Twop_mesons_HDF5, void *corrMesons, CORR_SPACE CorrSpace, bool HighMomForm){
...
template<typename Float>
void QKXTM_Contraction_Kepler<Float>::writeThrpHDF5(void *Thrp_local_HDF5, void *Thrp_noether_HDF5, void **Thrp_oneD_HDF5, char *filename, qudaQKXTMinfo_Kepler info, int isource, WHICHPARTICLE NUCLEON){
...
template<typename Float>
void QKXTM_Contraction_Kepler<Float>::writeThrpHDF5_PosSpace(void *Thrp_local_HDF5, void *Thrp_noether_HDF5, void **Thrp_oneD_HDF5, char *filename, qudaQKXTMinfo_Kepler info, int isource, WHICHPARTICLE NUCLEON){
...
template<typename Float>
void QKXTM_Contraction_Kepler<Float>::writeThrpHDF5_MomSpace(void *Thrp_local_HDF5, void *Thrp_noether_HDF5, void **Thrp_oneD_HDF5, char *filename, qudaQKXTMinfo_Kepler info, int isource, WHICHPARTICLE NUCLEON){

  if(info.CorrSpace!=MOMENTUM_SPACE && info.HighMomForm) errorQuda("writeThrpHDF5_MomSpace: Support for writing the three-point function only in momentum-space for low # of momenta!\n");
...
template<typename Float>
void QKXTM_Contraction_Kepler<Float>::copyThrpToHDF5_Buf(void *Thrp_HDF5, void *corrThp,  int mu, int uORd, int its, int Nsink, int pr, int sign, THRP_TYPE type, CORR_SPACE CorrSpace, bool HighMomForm){
...
void QKXTM_Contraction_Kepler<Float>::contractFixSink(QKXTM_Propagator_Kepler<Float> &seqProp,QKXTM_Propagator_Kepler<Float> &prop, QKXTM_Gauge_Kepler<Float> &gauge,
  ...
  if(CorrSpace==POSITION_SPACE){
    for(int it = 0 ; it < GK_localL[3] ; it++)
      run_fixSinkContractions((void*)corrThp_local, (void*)corrThp_noether, (void*)corrThp_oneD, fwdTex, seqTex, gaugeTex, testParticle, partflag, it, isource, sizeof(Float), CorrSpace);
  }
  else if(CorrSpace==MOMENTUM_SPACE){
