...
void quda::run_UploadToCuda(void* in,cudaColorSpinorField &qudaVec, int precision, bool isEven){
  dim3 blockDim( THREADS_PER_BLOCK , 1, 1);
  dim3 gridDim( (GK_localVolume + blockDim.x -1)/blockDim.x , 1 , 1);
...
void quda::run_DownloadFromCuda(void* out,cudaColorSpinorField &qudaVec, int precision, bool isEven){
  dim3 blockDim( THREADS_PER_BLOCK , 1, 1);
  dim3 gridDim( (GK_localVolume + blockDim.x -1)/blockDim.x , 1 , 1);
